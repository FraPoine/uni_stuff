...
def populate_database():  
    ...
    if not admin:
        cursor.execute('''
            ...
            ('admin', 'MegaComplicatedPasswordYouWillNeverGuessThis!', 42)
        ''')
        conn.commit()
    conn.commit()
    conn.close()
...
def image():
    ...
    while '%' in filename:
        try:
            _filename = urllib.parse.unquote(filename)
...
def admin():
    if 'user_id' in session:
        ...
        if user[1] == 'admin':
            ...
            return render_template('admin.html', usernames=usernames, username=session['username'], played_games=user[3])
        else:
            return redirect(url_for('index'))
    else:
        return redirect(url_for('login'))
